...
class Cnn5layer():
    ...
    def create_model(self):
      ...
      self.model.add(Conv2D(filters=16, kernel_size=2, input_shape=(self.num_rows, self.num_columns, self.num_channels), activation='relu'))
    ...
    def train(self, x_train, x_test, y_train, y_test, x_val=None, y_val=None, num_epochs = 10, num_batch_size = 32, checkpoint=None):
        ...
        start = datetime.now()
        self.model.fit(x_train, y_train, batch_size=num_batch_size, epochs=num_epochs, validation_data=(x_test, y_test), verbose=1, callbacks=[checkpointer])
